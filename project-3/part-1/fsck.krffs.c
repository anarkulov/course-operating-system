...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (node->magic != KRFFS_File_System_Magic) {
        fputs("Invalid signature node was found\n", stderr);
    ...
    do {
        if (previous_node > node){
            fputs("Invalid node link was found.\n", stderr);
        ...
        if(!krffs_is_node_in_file_system(&file_system, node)){
            fputs("The link was found out of file system.\n", stderr);
        ...
        if(node->magic != KRFFS_File_System_Magic){
            fputs("Invalid signature type was found.\n", stderr);
        ...
        if (node->type == KRFFS_Root_Node) {
            printf("* root node of size %"PRIu64"\n", node->size);
        } else if (node->type == KRFFS_Free_Node) {
            printf("* free node of size %"PRIu64"\n", node->size);
        } else if (node->type == KRFFS_Reserved_Node) {
            printf("* file node %s of size %"PRIu64"\n", node->name, node->size);
        } else {
            fputs("Unknown node type was found.", stderr);
