...
int main(int argc, char **argv)
{
    ...
    struct krffs_node *previous_node = node;
    struct krffs_node *next_node = node;

    do {        
        if(previous_node->type == KRFFS_Free_Node && node->type == KRFFS_Reserved_Node){
            uint64_t res_size = node->size;
            uint64_t res_prev_size = node->previous_node_size;

            uint64_t fr_size = previous_node->size;
            uint64_t fr_prev_size = previous_node->previous_node_size;

            memcpy(previous_node, node, res_size);                   
            previous_node->previous_node_size = fr_prev_size;  
            krffs_sync_mapping(previous_node, sizeof(*previous_node));

            node = krffs_get_next_node(&file_system, previous_node);
            node = krffs_initialize_free_node(node, fr_size, res_size);
            krffs_sync_mapping(node, sizeof(*node));

            next_node = krffs_get_next_node(&file_system, node);
            next_node->previous_node_size = fr_size;
            krffs_sync_mapping(next_node, sizeof(*next_node));

            printf("* swapping previous free and current reserved node\n");
        } else if (node->type == KRFFS_Free_Node && previous_node->type == KRFFS_Free_Node){
            previous_node->size += node->size;
            previous_node = node;
            
            krffs_sync_mapping(node, sizeof(*node));

            next_node = krffs_get_next_node(&file_system, node);
            next_node->previous_node_size = node->size;
            krffs_sync_mapping(next_node, sizeof(*next_node));
            
            printf("* merging current and next free node\n");
        } 

        previous_node = node; 
        node = krffs_get_next_node(&file_system, node);
